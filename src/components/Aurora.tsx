 ...
 * Based on React Bits (https://reactbits.dev/backgrounds/aurora)
 * by DavidHDev - Uses OGL WebGL library for GPU-accelerated aurora rendering
 */
...
vec3 colorRamp(ColorStop colors[4], float factor) {
  int index = 0;
  for (int i = 0; i < 3; i++) {
...
export default function Aurora({
  ...
  useEffect(() => {
    ...
    let cachedColorStops = (lastStopsRef ?? colorStops).map((hex: string) => {
      const c = new Color(hex);
      return [c.r, c.g, c.b];
    });
    ...
    const update = (t: number) => {
      ...
      if (propsRef.current.colorStops !== lastStopsRef) {
        lastStopsRef = propsRef.current.colorStops;
        cachedColorStops = (lastStopsRef ?? colorStops).map((hex: string) => {
          const c = new Color(hex);
          return [c.r, c.g, c.b];
        });
